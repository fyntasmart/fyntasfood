...
const Admin = ({ onLogout }: { onLogout: () => void }) => {
  ...
  // Return Request Approve (Admin se)
  ...
  // Tier, Banner, Content, Invoice Functions
  const addTier = async () => {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f8f9fa', color: '#111111', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div className="content">
        ...
        {/* ... (Baaki tabs ka code pichle wala hi hai) ... */}
        {/* NOTE: Aapke paas pichle Admin.tsx mein saare tabs hain, bas upar wala Orders tab replace karna hai. */}
        {activeTab === 'products' && <div className="panel"><h3>Products</h3>{products.map(p => <div key={p.id}>{p.name}</div>)}</div>}
        {activeTab === 'categories' && <div className="panel"><h3>Categories</h3>{categories.map(c => <div key={c.id}>{c.name}</div>)}</div>}
        {activeTab === 'customers' && <div className="panel"><h3>Customers</h3>{customers.map(c => <div key={c.id}>{c.name} - {c.mobile}</div>)}</div>}
        ...
        {activeTab === 'charges' && <div className="panel"><h3>Charges</h3>{tiers.map(t => <div key={t.id}>{t.min_km} - {t.max_km}: ₹{t.price}</div>)}</div>}
        {activeTab === 'banners' && <div className="panel"><h3>Banners</h3>{banners.map(b => <div key={b.id}>{b.title}</div>)}</div>}
        {activeTab === 'invoice_settings' && <div className="panel"><h3>Invoice Settings</h3><p>Welcome Note: {invoiceSettings.welcome_note}</p></div>}
        {activeTab === 'profile' && <div className="panel"><h3>Profile</h3></div>}
        {activeTab === 'policies' && <div className="panel"><h3>Policies</h3></div>}
        {activeTab === 'content' && <div className="panel"><h3>Content</h3><p>{currentContent}</p></div>}
      </div>
    </div>
  );
};
