...
const Admin = () => {
  ...
  // Branch ki Range Update karna
  const updateRange = async (branchId: string, newRange: number) => {
    const { error } = await supabase
      .from('branches')
      .update({ delivery_range_km: newRange })
      .eq('id', branchId);

    if (!error) {
      setBranches(
        branches.map((b) =>
          b.id === branchId ? { ...b, delivery_range_km: newRange } : b
        )
      );
          ...
          <label>
            ...
            <input
              ...
              onChange={(e) =>
                setSettings({
                  ...settings,
                  per_km_charge: parseFloat(e.target.value),
                })
              }
              style={{ marginLeft: '5px', width: '80px' }}
            />
          ...
          <label>
            ...
            <input
              ...
              onChange={(e) =>
                setSettings({
                  ...settings,
                  max_delivery_km: parseFloat(e.target.value),
                })
              }
              style={{ marginLeft: '5px', width: '80px' }}
            />
          ...
          >
            <div>
              ...
              <span style={{ fontSize: '14px', color: '#666' }}>
                Current Range: {branch.delivery_range_km} KM
              </span>
            </div>
            ...
            <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
              ...
              <label style={{ fontSize: '14px' }}>
                ...
                <input
                  type="number"
                  defaultValue={branch.delivery_range_km}
                  onBlur={(e) =>
                    updateRange(branch.id, parseFloat(e.target.value))
                  }
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: '5px',
                  cursor: 'pointer',
                  background: branch.is_active ? 'green' : 'red',
                  color: 'white',
                  fontWeight: 'bold',
                }}
              >
