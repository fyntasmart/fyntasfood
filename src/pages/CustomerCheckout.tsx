...
const CustomerCheckout = ({ cart, onSuccess, onBack }: any) => {
  ...
  const handleCodButton = () => handlePlaceOrder('pending', 'cod', null);
          ...
          <>
            ...
            <div style={{ marginBottom: '15px' }}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                ...
                <button onClick={() => setPayMethod('cod')} style={{ flex: 1, padding: '12px', borderRadius: '8px', border: payMethod === 'cod' ? '2px solid #1e40af' : '1px solid #ccc', background: payMethod === 'cod' ? '#dbeafe' : '#fff', color: '#111827', fontWeight: 'bold', cursor: 'pointer' }}>COD</button>
              </div>
            </div>
            ...
            <div style={{ borderTop: '2px solid #e5e7eb', paddingTop: '15px' }}>
              ...
              {payMethod === 'upi' && <button onClick={handlePayButton} disabled={loading} style={{ width: '100%', padding: '16px', background: '#1e40af', color: '#fff', border: 'none', borderRadius: '10px', fontSize: '18px', fontWeight: 'bold', cursor: 'pointer', marginTop: '15px' }}>{loading ? 'Placing...' : 'Pay via UPI / QR'}</button>}
      ...
      {showUpiModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }} onClick={() => setShowUpiModal(false)}>
          <div style={{ background: '#fff', borderRadius: '15px', padding: '20px', maxWidth: '350px', width: '100%', textAlign: 'center' }} onClick={(e) => e.stopPropagation()}>
            ...
            <button onClick={handleConfirmUpiPayment} style={{ width: '100%', padding: '12px', background: '#1e40af', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}>✓ I have paid - Place Order</button>
